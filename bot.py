...
from PIL import Image, ImageFile  # https://pypi.org/project/Pillow/
# Webhook stuffs
from discord_webhook import DiscordWebhook, DiscordEmbed
# HTTP server/static modules
from flask import Flask, abort, jsonify  # https://pypi.org/project/Flask/
...
def player_on_map(map_data: tuple):
    ...
    if not on_map and args.d:
        debug_log.info(
            f"Player was not on target map of {map_data[0]},{map_data[1]}. Map was {trainer_info['mapBank']}, {trainer_info['mapId']}")
...
def opponent_changed():  # This function checks if there is a different opponent since last check, indicating the game state is probably now in a battle
    try:
        ...
        if opponent_info and last_opponent_personality != opponent_info["personality"]:
            debug_log.info(
                f"Opponent has changed! Previous PID: {last_opponent_personality}, New PID: {opponent_info['personality']}")
...
def get_screenshot():
    ...
    try:
        ...
        g_bizhawk_screenshot = cv2.cvtColor(numpy.array(screenshot),
                                            cv2.COLOR_BGR2RGB)  # Convert screenshot to numpy array COLOR_BGR2RGB
        screenshot.close()
    except:
...
def find_image(file: str):  # Function to find an image in a BizHawk screenshot
    try:
        ...
        correlation = cv2.matchTemplate(g_bizhawk_screenshot, template[:, :, 0:3],
        ...
        if max_val_corr > threshold:
            if args.di:
                ...
                for point in zip(*loc[::-1]):
                    ...
                    cv2.waitKey(1)
            if args.di: debug_log.debug(
                f"Maximum correlation value ({max_val_corr}) is above threshold ({threshold}), file {file} was on-screen!")
            return True
        else:
            if args.di: debug_log.debug(
                f"Maximum correlation value ({max_val_corr}) is below threshold ({threshold}), file {file} was not detected on-screen.")
            return False
...
def catch_pokemon():  # Function to catch pokemon
    try:
        ...
        if config["manual_catch"]:
            input(
                "Pausing bot for manual catch (don't forget to pause pokebot.lua script so you can provide inputs). Press Enter to continue...")
            return True
        else:
...
def log_encounter(pokemon: dict):
    ...
    def common_stats():
        ...
        write_file("stats/encounter_log.json",
    ...
    debug_log.info(
        f"Shiny Value (SV): {pokemon['shinyValue']:,} (is {pokemon['shinyValue']:,} < 8 = {pokemon['shiny']})")
    ...
    if pokemon["shiny"]:
        ...
        if config["discord_webhook_url"]:
            ...
            if config["discord_shiny_ping"] and config[
                "discord_ping_mode"] == "role":  # Thanks Discord for making role and user IDs use the same format, but have different syntaxes for pinging them by ID, really cool.
            ...
            embed = DiscordEmbed(title='Shiny encountered!',
                                 description=f"{pokemon['name']} at {pokemon['metLocationName']}", color='ffd242')
            ...
            embed.add_embed_field(name='Species Phase Encounters',
                                  value=f"{stats['pokemon'][mon_name]['phase_encounters']}")
...
def identify_pokemon(
        starter: bool = False):  # Identify opponent pokemon and incremement statistics, returns True if shiny, else False
...
def enrich_mon_data(pokemon: dict):  # Function to add information to the pokemon data extracted from Bizhawk
    try:
        ...
        pokemon["shinyValue"] = int(bin(personalityF ^ personalityL ^ trainer_info["tid"] ^ trainer_info["sid"])[2:],
...
def mode_spin():
    ...
    while True:
        ...
        if home_coords != (trainer_info["posX"], trainer_info[
            "posY"]):  # Note: this will likely fail if the trainer accidentally changes maps
...
def mode_castform():
    ...
    if (not player_on_map(MapDataEnum.ROUTE_119_B.value) or not (
...
def collect_gift_mon(target: str):
    ...
    while True:
        ...
        write_file(f"stats/{trainer_info['tid']}/{target.lower()}.json",
...
def mode_fishing():
    ...
    while not opponent_changed():
        ...
        if not find_image("text_period.png"): emu_combo(
...
def mode_starters():
    ...
    while True:
        ...
        while emu_info["rngState"] in starter_frames["rngState"]:
            ...
            write_file(f"stats/{trainer_info['tid']}/{choice}.json",
...
def mode_farawayMew():
    if (not player_on_map(MapDataEnum.FARAWAY_ISLAND.value) or not (
            22 <= trainer_info["posX"] <= 23 and 8 <= trainer_info["posY"] <= 10)):
...
def mode_reporting():
    debug_log.info(
        "Running in reporting mode... Don't forget to set `enable_input` to false in `pokebot.lua` before running in Bizhawk!")
...
try:
    ...
    parser.add_argument('-m',
                        action='store_true')  # -m flag: set to Manual Mode (bot will check for shinies and update stats without providing any button input)
    ...
    log_handler = RotatingFileHandler(logFile, mode='a', maxBytes=20 * 1024 * 1024, backupCount=5, encoding=None,
                                      delay=0)
...
try:
    ...
    last_trainer_state, last_opponent_personality, trainer_info, opponent_info, emu_info, party_info, emu_speed, language, MapDataEnum = None, None, {}, {}, {}, [], 1, None, None
    ...
    if can_start_bot:
        ...
        default_input = {"A": False, "B": False, "L": False, "R": False, "Up": False, "Down": False, "Left": False,
                         "Right": False, "Select": False, "Start": False, "Light Sensor": 0, "Power": False,
                         "Tilt X": 0, "Tilt Y": 0, "Tilt Z": 0, "SaveRAM": False}
    ...
    stats = json.loads(totals) if totals else {"pokemon": {}, "totals": {"longest_phase_encounters": 0,
                                                                         "shortest_phase_encounters": "-",
                                                                         "phase_lowest_sv": 99999,
                                                                         "phase_lowest_sv_pokemon": "", "encounters": 0,
                                                                         "phase_encounters": 0, "shiny_average": "-",
                                                                         "shiny_encounters": 0}}
    ...
    window = webview.create_window("PokeBot", url=url, width=1280, height=720, resizable=True, hidden=False,
                                   frameless=False, easy_drag=True, fullscreen=False, text_select=True, zoomable=True)
